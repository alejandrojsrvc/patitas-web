  ...
  return (
    <div className="lg:hidden">
      ...
      {isOpen ? (
        <div
          id="mobile-menu"
          className="absolute inset-x-0 top-full max-h-[calc(100svh-8.25rem)] overflow-y-auto bg-catalog-canvas px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 sm:px-4"
        >
          <Link
            href="/mi-cuenta"
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="block truncate text-sm font-semibold">{displayAddress ?? "Elegí tu dirección"}</span>
            </span>
          </Link>
          ...
          <nav aria-label="Navegación mobile" className="grid gap-1">
            {links.map(([label, href]) => (
              <Link
                ...
                href={href}
                onClick={() => setIsOpen(false)}
                className="min-h-11 rounded-xl px-3 py-2.5 font-semibold text-ink hover:bg-soft-blue hover:text-brand-blue"
              >
                {label}
              </Link>
            ))}
              ...
              <div className="mt-1 grid grid-cols-2 gap-1">
                {categoryLinks.map(([label, href]) => (
                  <Link
                    ...
                    href={href}
                    onClick={() => setIsOpen(false)}
                    className="min-h-10 rounded-xl px-3 py-2 text-sm font-semibold text-ink hover:bg-soft-blue hover:text-brand-blue"
                  >
                    {label}
                  </Link>
                ))}
            ...
            <Link
              href="/mi-cuenta"
              onClick={() => setIsOpen(false)}
              className="mt-1 flex min-h-12 items-center gap-3 rounded-xl px-3 py-3 font-semibold text-ink"
            >
              <UserCircle size={22} weight="bold" /> {displayName ? "Ver mi cuenta" : "Iniciar sesión"}
            </Link>
          </nav>
        </div>
      ) : null}
