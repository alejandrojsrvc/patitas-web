...
async function CachedBrandPage({
  ...
  const schema = {
    ...
    "@graph": [
      {
        "@type": "CollectionPage",
        name: `${brand.name}: productos para perros y gatos`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 3, name: brand.name, item: `${siteUrl}/marcas/${brand.slug}` },
        ],
      },
      {
        "@type": "ItemList",
        ...
        <section className="bg-soft-blue py-8 sm:py-14 lg:py-20">
          <div className="container-shell">
            <nav aria-label="Migas de pan" className="mb-5 text-xs text-muted sm:text-sm">
              ...
              <Link href="/marcas" className="hover:text-brand-blue hover:underline">
                Marcas
              </Link>
              <span aria-hidden="true"> / </span>
            ...
            <h1 className="display-heading text-4xl sm:text-7xl">{brand.name}</h1>
            <p className="mt-4 max-w-2xl text-base text-muted sm:mt-5 sm:text-lg">
              {brand.description ?? "Todas las presentaciones activas de la marca, con precio y disponibilidad actual."}
            </p>
          </div>
        </section>
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }} />
        <section className="container-shell py-7 sm:py-12">
        ...
        {products.items.length ? (
          <section className="bg-page-bg py-12 sm:py-16">
            <div className="container-shell">
              ...
              <div className="mt-6 flex flex-wrap gap-x-5 gap-y-3 text-sm font-semibold text-brand-blue">
                <Link href="/calculadora-alimento" className="hover:underline">
                  Probar calculadora →
                </Link>
                {hasDogProducts ? (
                  <Link href="/guias/cuanto-alimento-come-un-perro" className="hover:underline">
                    Cuánto alimento come un perro →
                  </Link>
                ) : null}
                {hasCatProducts ? (
                  <Link href="/guias/cuanto-alimento-come-un-gato" className="hover:underline">
                    Cuánto alimento come un gato →
                  </Link>
                ) : null}
              </div>
            </div>
