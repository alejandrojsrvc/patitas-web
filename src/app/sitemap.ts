...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...indexableCatalogRoutes.map((route) => ({ url: `${siteUrl}${catalogPath(route.species, route.segments)}`, changeFrequency: "daily" as const, priority: 0.8 })),
    ...(brands.ok ? brands.data.map((brand) => ({ url: `${siteUrl}/marcas/${brand.slug}`, changeFrequency: "weekly" as const, priority: 0.7 })) : []),
    ...(products.ok ? products.data.map((product) => ({ url: `${siteUrl}/producto/${product.slug}`, lastModified: new Date(product.updatedAt), changeFrequency: "daily" as const, priority: 0.8 })) : []),
