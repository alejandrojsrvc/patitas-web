...
import { authenticatedApiCall } from "@/lib/auth-session";
...
export async function GET() {
  try {
    ...
    const response = NextResponse.json({ message: error instanceof Error ? error.message : "Necesitás iniciar sesión." }, { status: error instanceof AuthApiError && error.status !== 401 ? error.status : 401 });
