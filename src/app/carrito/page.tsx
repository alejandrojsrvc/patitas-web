  ...
  const checkoutMessage = result.error ?? (checkoutError === "checkout-unavailable"
    ...
    : checkoutError === "checkout-conflict"
      ...
      : checkoutError === "cart-unavailable"
        ...
        : undefined);
  return <><CartHydrator cart={screen?.cart ?? shell.cart} /><SiteHeader initialShell={shell} /><main id="contenido" className="min-h-[65vh] bg-catalog-canvas py-7 sm:py-12"><div className="container-shell"><h1 className="display-heading text-4xl sm:text-6xl">Carrito</h1><p className="mt-3 max-w-xl text-base text-muted sm:text-lg">Revisá tus presentaciones y cantidades antes de confirmar el pedido.</p><div className="mt-7 sm:mt-9"><CartPageContent checkoutMessage={checkoutMessage} /></div></div></main><SiteFooter /></>;
}

function CartPageFallback() {
  return <><CartHydrator cart={emptyStorefrontShell.cart} /><SiteHeader initialShell={emptyStorefrontShell} /><main id="contenido" className="min-h-[65vh] bg-catalog-canvas py-7 sm:py-12"><div className="container-shell"><h1 className="display-heading text-4xl sm:text-6xl">Carrito</h1><div className="mt-7 rounded-xl bg-white p-8 text-center text-muted">Cargando tu carrito…</div></div></main><SiteFooter /></>;
}

function CartRefreshFailure() {
  return <><CartHydrator cart={emptyStorefrontShell.cart} /><SiteHeader initialShell={emptyStorefrontShell} /><main id="contenido" className="min-h-[65vh] bg-catalog-canvas py-7 sm:py-12"><div className="container-shell rounded-xl bg-white p-8"><h1 className="font-display text-2xl font-semibold">No pudimos renovar tu sesión</h1><p className="mt-2 text-muted">Tu carrito anónimo se conserva. Iniciá sesión nuevamente para recuperar el carrito de tu cuenta.</p></div></main><SiteFooter /></>;
}
