...
function formatWeight(weightGrams: number | null) {
  ...
  return weightGrams >= 1000 ? `${Number((weightGrams / 1000).toFixed(2))} kg` : `${weightGrams} g`;
}

export function productDisplayName(product: Product) {
  const productName = product.name.trim();
  const brandName = product.brand.name.trim();
  const nameWithBrand = normalized(productName).includes(normalized(brandName)) ? productName : `${brandName} ${productName}`;
  const purchasableVariants = product.variants.filter((variant) => variant.fulfillment.purchasable);
  const onlyWeight = purchasableVariants.length === 1 ? formatWeight(purchasableVariants[0].weightGrams) : null;
