...
export function isIdempotencyConflict(error: unknown) {
  return Boolean(error && typeof error === "object" && "status" in error && error.status === 409 && "code" in error && error.code === "PAYMENT_IDEMPOTENCY_CONFLICT");
}
...
export function paymentStatusLabel(status: PaymentStatus) {
  ...
  } as Record<string, string>)[status] ?? "Estado de pago no disponible";
}

export function paymentStatusMessage(status: PaymentStatus) {
  if (isPaymentPollingStatus(status)) return "Estamos confirmando tu pago. No cierres esta ventana.";
