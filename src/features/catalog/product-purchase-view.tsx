...
export function ProductPurchaseView({ product }: { product: ProductDetail }) {
  ...
  const variant = useMemo(() => product.variants.find((item) => item.id === selectedVariantId) ?? selectInitialVariant(product), [product, selectedVariantId]);
  ...
  return (
    <>
      <div className="mt-5 grid items-start gap-7 lg:mt-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] lg:gap-x-10 lg:gap-y-8 xl:grid-cols-[minmax(0,1.6fr)_minmax(0,1.4fr)_minmax(17rem,1fr)] xl:gap-6">
        <div className="lg:row-span-2 xl:row-span-1"><ProductGallery key={selectedVariantId} media={product.media} productName={product.name} selectedVariantId={selectedVariantId} /></div>
