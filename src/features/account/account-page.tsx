...
export function AccountPage({ section, orderId }: { section: AccountSection; orderId?: string }) {
  ...
  const [plans, setPlans] = useState<ReplenishmentPlanSummary[]>([]);
  ...
  const loadAccount = useCallback(async () => {
    ...
    try {
      ...
      if (section === "mascotas") setPets(await requestJson<CustomerPet[]>("/me/pets"));
      if (section === "reposiciones") setPlans(await requestJson<ReplenishmentPlanSummary[]>("/replenishment-plans"));
  ...
  async function handleLogout() {
    ...
    try {
      ...
      setUser(null);
      setProfile(null);
      window.dispatchEvent(new Event("patitas-auth-changed"));
    } catch (cause) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10">
    ...
    <section className="min-w-0"><h1 className="display-heading text-3xl sm:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-base text-muted">Gestioná tus datos, pedidos y preferencias desde un solo lugar.</p>{error ? <p role="alert" className="mt-5 rounded-xl bg-[#fff1f1] p-4 text-sm text-[#8d2020]">{error}</p> : null}{message ? <p role="status" className="mt-5 rounded-xl bg-soft-blue p-4 text-sm text-ink">{message}</p> : null}{section === "resumen" ? <Summary profile={profile} orders={orders} onProfileChange={setProfile} setMessage={setMessage} setError={setError} /> : null}{section === "pedidos" ? <Orders orders={orders} detail={order} /> : null}{section === "direcciones" ? <Addresses addresses={addresses} onChange={setAddresses} setMessage={setMessage} /> : null}{section === "mascotas" ? <Pets pets={pets} onChange={setPets} setMessage={setMessage} setError={setError} /> : null}{section === "reposiciones" ? <Replenishments plans={plans} setMessage={setMessage} setError={setError} /> : null}</section>
  </div>;
}
...
function Summary({ profile, orders, onProfileChange, setMessage, setError }: { profile: CustomerProfile | null; orders: OrderSummary[]; onProfileChange: (profile: CustomerProfile) => void; setMessage: (message: string | null) => void; setError: (message: string | null) => void }) {
...
function Pets({ pets, onChange, setMessage, setError }: { pets: CustomerPet[]; onChange: (pets: CustomerPet[]) => void; setMessage: (message: string | null) => void; setError: (message: string | null) => void }) {
  ...
  return <div className="mt-7 grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="grid gap-3">{pets.map((pet) => <article key={pet.id} className="rounded-xl bg-white p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-display text-xl font-semibold">{pet.name}</h2><p className="mt-1 text-muted">{pet.species === "dog" ? "Perro" : "Gato"} · {pet.weightKg} kg</p><p className="text-sm text-muted">{lifeStageLabel(pet.lifeStage)}{pet.breed ? ` · ${pet.breed}` : ""}</p></div><PawPrint size={24} className="shrink-0 text-brand-blue" /></div><button type="button" onClick={() => setEditing(pet)} className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-brand-blue focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue"><PencilSimple size={16} /> Editar</button></article>)}{!pets.length ? <div className="rounded-xl bg-white p-7"><PawPrint size={30} className="text-brand-blue" /><h2 className="mt-3 font-display text-xl font-semibold">Agregá tu primera mascota</h2><p className="mt-1 text-muted">Sus datos se usan para personalizar recomendaciones y reposiciones.</p></div> : null}</div><form key={editing?.id ?? "new-pet"} onSubmit={save} className="h-fit rounded-xl bg-white p-5"><h2 className="font-display text-xl font-semibold">{editing ? `Editar a ${editing.name}` : "Nueva mascota"}</h2><div className="mt-4 grid gap-3"><AddressField name="name" label="Nombre" defaultValue={editing?.name} /><SelectField name="species" label="Especie" defaultValue={editing?.species ?? "dog"} options={[["dog", "Perro"], ["cat", "Gato"]]} /><AddressField name="weightKg" label="Peso (kg)" inputMode="decimal" pattern="[0-9]+([.,][0-9]+)?" defaultValue={editing?.weightKg} /><SelectField name="lifeStage" label="Etapa" defaultValue={editing?.lifeStage ?? "adult"} options={[["puppy", "Cachorro"], ["adult", "Adulto"], ["senior", "Senior"]]} /><AddressField name="breed" label="Raza" defaultValue={editing?.breed ?? ""} required={false} /></div><div className="mt-5 flex gap-2"><button type="submit" disabled={saving} className="inline-flex min-h-11 flex-1 items-center justify-center rounded-xl bg-brand-blue px-4 font-semibold text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-blue disabled:opacity-60">{saving ? "Guardando…" : "Guardar"}</button>{editing ? <button type="button" onClick={() => setEditing(null)} className="min-h-11 rounded-xl bg-catalog-canvas px-4 font-semibold">Cancelar</button> : null}</div></form></div>;
}
...
function AccountSkeleton() { return <div className="grid animate-pulse gap-6 lg:grid-cols-[240px_minmax(0,1fr)] lg:gap-10" role="status" aria-label="Cargando tu cuenta"><div className="h-72 rounded-xl bg-white" /><div><div className="h-10 w-52 rounded-lg bg-white" /><div className="mt-3 h-5 max-w-xl rounded bg-white" /><div className="mt-7 grid gap-4 sm:grid-cols-2"><div className="h-52 rounded-xl bg-white" /><div className="h-52 rounded-xl bg-white" /></div></div></div>; }
