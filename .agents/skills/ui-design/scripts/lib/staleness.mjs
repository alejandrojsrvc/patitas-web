...
const KNOWN_CONFIG_KEYS = new Set(["hook", "detector", "updateCheck", "stalenessCheck", "projectRoots", "buildPath", "$schema", "version"]);
...
const DIRECTION_WORK_PATHS = Object.freeze([path.join(".uizze", "surfaces"), path.join(".uizze", "mocks", "decision")]);
...
export function designSidecarCandidatesFor(projectRoot, contextDir = projectRoot) {
  const candidates = [path.join(projectRoot, ".uizze", "design.json"), path.join(projectRoot, "DESIGN.json")];
 ...
 */
export function checkProjectRoots({ patterns = [], candidates = [], configuredIn = ".uizze/config.json" }) {
