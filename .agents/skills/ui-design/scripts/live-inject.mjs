...
import { describeInjectArtifacts, frameworkIgnorePatterns, resolveFramework, resolveSourceTraits } from "./live/frameworks/index.mjs";
...
Output (JSON):
  ...
  if (!token) {
    try {
      const info = JSON.parse(fs.readFileSync(path.join(cwd, ".uizze", "live", "server.json"), "utf-8"));
  ...
  try {
    const rootsManifest = JSON.parse(fs.readFileSync(path.join(cwd, ".uizze", "live", "roots.json"), "utf-8"));
...
export { applyNuxtLiveAdapter, buildNuxtPlugin, detectNuxtProject, removeNuxtLiveAdapter } from "./live/frameworks/nuxt.mjs";
