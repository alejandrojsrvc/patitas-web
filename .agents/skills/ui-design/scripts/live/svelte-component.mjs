...
export const DEFERRED_ACCEPTS_FILE = ".uizze/live/deferred-svelte-component-accepts.json";

const MUSTACHE_RE = /\{([^{}]+)\}/g;

export function shouldUseSvelteComponentInjection(filePath) {
  if (/^(0|false|no)$/i.test(process.env.UIZZE_LIVE_SVELTE_COMPONENT || "")) return false;
...
export function ensureRuntimeHelper(cwd = process.cwd()) {
  ...
  if (!fs.existsSync(probe)) {
    fs.writeFileSync(probe, `export const uizzeLivePreviewProbe = true;\n`, "utf-8");
  }
  return file;
}
...
function appendSanitizedCssRule(output, rule, variantNum, paramValues, rootTag) {
  ...
  if (/^@scope\b/i.test(prelude)) {
    if (/data-uizze-variant/.test(prelude) && !selectorHasVariant(prelude, variantNum)) return;
    ...
    for (const innerRule of inner) {
      ...
      if (!rewrittenPrelude || /--uizze-variant-ready\s*:/.test(innerRule.body)) continue;
...
function variantSelectorRegex(variantNum) {
  return new RegExp(`\\[data-uizze-variant=(["'])${escapeRegExp(String(variantNum))}\\1\\]`, "g");
}
...
function inlineSvelteComponentInsertAccept({ manifest, markup, cssLines, variantNum, paramValues, sourceFile, resultBase, cwd }) {
  ...
  if (/\bdata-uizze-[\w-]*\s*=/.test(markup)) {
    return { handled: false, error: "Accepted Svelte insert variant contains preview-only data-uizze attributes", ...resultBase };
  }
...
export function deferredAcceptsPath(cwd = process.cwd()) {
  ...
  return path.join(os.tmpdir(), "uizze-live", key, "deferred-svelte-component-accepts.json");
}
