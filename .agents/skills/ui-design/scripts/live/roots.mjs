...
function isAppRoot(dir) {
  ...
  return hasDevConfig(dir) || exists(path.join(dir, ".uizze", "live", "config.json"));
}
...
export function resolveRoots({ cwd = process.cwd(), targetPath = null } = {}) {
  ...
  const markerBound = absTarget && insideOrEqual(targetDir, legacyRoot) && insideOrEqual(legacyRoot, upperBound) ? legacyRoot : upperBound;
...
export function writeRootsManifest(manifest) {
  ...
  if (path.resolve(manifest.repoRoot) !== path.resolve(manifest.appRoot)) {
    ...
    const entries = readPointerEntries(manifest.repoRoot).filter((entry) => path.resolve(entry.appRoot) !== path.resolve(manifest.appRoot));
...
function hasLiveServer(appRoot) {
  ...
  let token;
  try {
    const info = JSON.parse(fs.readFileSync(path.join(appRoot, ".uizze", "live", "server.json"), "utf-8"));
  ...
  if (!targetPath) {
    ...
    if (gitRoot) {
      ...
      if (candidates.length > 0) {
        ...
        const recoveringApps = liveApps.length > 0 ? liveApps : candidates.filter((manifest) => hasActiveDurableSession(manifest.appRoot));
...
export function enterLiveRoot(cwd = process.cwd()) {
  ...
  if (path.resolve(cwd) !== path.resolve(appRoot)) {
    ...
    try {
      ...
      console.error(`[uizze live] could not enter app root ${appRoot}: ${err.message}`);
      process.exit(1);
    }
  }
