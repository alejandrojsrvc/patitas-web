...
Output (JSON):
  ...
  if (svelteComponentManifest) {
    ...
    let result;
    try {
      result = withSourceLockSync(path.resolve(process.cwd(), sourceFile), requestedOperation + ":" + id, runOperation, {
...
function buildCarbonizeReplacement({ indent, commentSyntax, isJsx, id, variantNum, cssContent, paramValues, restored }) {
  ...
  const pushCarbonizeBody = (bodyIndent) => {
    ...
    lines.push(bodyIndent + commentSyntax.open + " uizze-carbonize-start " + id + " " + commentSyntax.close);
    lines.push(bodyIndent + '<style data-uizze-css="' + id + '">' + (isJsx ? "{`" : ""));
    ...
    if (paramValues && Object.keys(paramValues).length > 0) {
      lines.push(
        bodyIndent + commentSyntax.open + " uizze-param-values " + id + ": " + JSON.stringify(paramValues) + " " + commentSyntax.close,
      );
    }
    lines.push(bodyIndent + commentSyntax.open + " uizze-carbonize-end " + id + " " + commentSyntax.close);
    lines.push(bodyIndent + '<div data-uizze-variant="' + variantNum + '" ' + variantStyleAttr + ">");
  ...
  if (isJsx) {
    ...
    lines.push(indent + '<div data-uizze-carbonize="' + id + '" ' + wrapperStyle + ">");
...
function readSourceShadowPreviewMeta(content, id) {
  ...
  const wrapperRe = new RegExp("<[^>]+data-uizze-variants=([\"'])" + escaped + "\\1[^>]*>");
...
function hasVariantWrapperAttr(line, id) {
  const escaped = escapeRegExp(id);
  return new RegExp(`data-uizze-variants\\s*=\\s*(?:"${escaped}"|'${escaped}'|\\{["']${escaped}["']\\})`).test(line);
}
