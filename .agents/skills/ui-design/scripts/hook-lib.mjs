...
export const HOOK_LOCAL_IGNORE_PATTERNS = Object.freeze([".uizze/hook.cache.json", ".uizze/hook.pending.json", ".uizze/config.local.json"]);
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0 ? `... and ${remaining} more (see ${UIZZE_COMMAND} audit).` : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see ${UIZZE_COMMAND} audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted, footerText) =>
    [header, ...linesArr, ...(omitted ? [`... and more (see ${UIZZE_COMMAND} audit).`] : []), "", footerText].join("\n");
  ...
  return clampLastLine((linesArr, footerText) => assemble(linesArr, true, footerText), lines.find(isFindingLine) || lines[0], maxChars);
}
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  return clampLastLine((linesArr, footerText) => assemble(linesArr, lastMore, footerText), lines.find(isFindingLine) || lines[0], maxChars);
}
...
const STEER_LINE = "That does not mean the design is good: keep following the project design system and the uizze skill guidance.";
...
export async function runHook({ stdinJson, env = {}, cwd = process.cwd(), now = Date.now, detector } = {}) {
  ...
  try {
    ...
    if (deferredTotal > 0 || (cacheDirty && fs.existsSync(path.join(projectCwd, ".uizze")))) {
      persistCache(projectCwd, cache);
    }
