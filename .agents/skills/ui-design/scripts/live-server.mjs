...
import { LIVE_COMMANDS, VARIANT_PROGRESS_CHECKPOINT_REASONS as VARIANT_PROGRESS_CHECKPOINT_REASON_LIST } from "./live/vocabulary.mjs";
import {
...
function loadBrowserScripts() {
  ...
  const detectPaths = [
    ...
    path.join(__dirname, "..", "..", "..", "..", "cli", "engine", "detect-antipatterns-browser.js"),
    path.join(process.cwd(), "node_modules", "uizze", "cli", "engine", "detect-antipatterns-browser.js"),
  ];
  let detectScript = "";
  ...
  return (req, res) => {
    ...
    if (p === "/events" && req.method === "POST") {
      ...
      req.on("end", () => {
        ...
        if (msg.id && state.sessionStore && !SESSION_CREATING_EVENT_TYPES.has(msg.type) && !state.sessionStore.has(msg.id)) {
          res.writeHead(404, { "Content-Type": "application/json" });
...
function cleanupSvelteComponentSessionsBeforeExit() {
  try {
    ...
    console.warn("[uizze] Svelte component session cleanup failed:", err.message);
  }
}
...
function sweepOrphanSvelteComponentSessionsOnStartup() {
  try {
    ...
    if (result.removed.length > 0 || result.removedRoot) {
      console.log("[uizze] swept orphaned Svelte component sessions:", JSON.stringify(result));
    }
  } catch (err) {
...
function sweepStaleAcceptReceiptsOnStartup() {
  try {
    ...
    if (removed > 0) console.log(`[uizze] removed ${removed} accept receipt(s) older than 14 days`);
  } catch (err) {
...
function applyLegacyDeferredAcceptsOnStartup() {
  try {
    ...
    if (result.applied > 0 || result.failed > 0) {
      console.log("[uizze] applied legacy deferred Svelte component accepts:", JSON.stringify(result));
    }
  } catch (err) {
    console.warn("[uizze] legacy deferred Svelte component accept apply failed:", err.message);
  }
}
