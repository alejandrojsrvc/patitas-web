...
function readProjectRootPatterns(repoRoot) {
  ...
  for (const name of ["config.json", "config.local.json"]) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(repoRoot, ".uizze", name), "utf-8"));
...
function renderText(report, fixes) {
  ...
  if (fixes) {
    ...
    lines.push(
      `Run \`node doctor.mjs --fix\` to apply the automatic migrations, ` + `or \`${UIZZE_COMMAND} doctor\` to work through all of them.`,
    );
  }
