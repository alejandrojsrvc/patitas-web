        ...
        options: [
          {
            ...
            raised: [{ from: "challenger-microfiche", raise: "The bill now owns its whole viewport as one continuous printed sheet." }],
            ...
            board: "https://uizze.style/worlds/cards/posters-covers-sleeves-fillmore-handbill.webp",
          },
          {
            id: "model-pick",
  ...
  const anatomy = (option) => {
    ...
    if (Array.isArray(option.raised) && option.raised.length) {
      ...
      const raiseLines = option.raised
        .slice(0, 6)
        .map((r) => `<p class="raise"><span class="fact-label">From ${esc(nameOf(r.from))}</span>${esc(r.raise || r.kept || "")}</p>`);
