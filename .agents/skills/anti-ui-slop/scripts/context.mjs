...
const UPDATE_HOST = (process.env.UIZZE_UPDATE_HOST || "https://uizze.style").replace(/\/$/, "");
const UPDATE_CACHE_PATH = process.env.UIZZE_UPDATE_CACHE || path.join(os.homedir(), ".uizze", "update-check.json");
...
function updateCheckDisabledByConfig(cwd = process.cwd()) {
  ...
  for (const name of ["config.json", "config.local.json"]) {
    try {
      const raw = JSON.parse(fs.readFileSync(path.join(cwd, ".uizze", name), "utf-8"));
...
function valueHasHookMarker(value) {
  if (typeof value === "string") {
    return value.includes("skills/uizze/scripts/hook.mjs") || value.includes("skills/uizze/scripts/hook-before-edit.mjs");
  }
  ...
  for (const root of roots) {
    for (const rel of manifests) {
      ...
      if (raw?.hooks && valueHasHookMarker(raw.hooks)) {
        return STOP_REVIEW_PROVIDERS.has(UIZZE_PROVIDER_ID) ? "stop" : "per-edit";
      }
    }
  }
  return "none";
