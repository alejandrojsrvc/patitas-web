...
const API_BASE = (process.env.UIZZE_API_URL || "https://uizze.style/api").replace(/\/$/, "");
...
function requireLocalConcepts() {
  ...
  if (!local) {
    throw new Error("concept-seed: no local catalog (set UIZZE_CATALOG_DIR or pass sourceConcepts)");
  }
  return local;
}
...
const CARD_BASE = process.env.UIZZE_CARD_BASE || "https://uizze.style/worlds/cards";
    ...
    if (chosenIdx !== -1 || kindIdx !== -1) {
      ...
      process.stdout.write(
        await renderConceptSeed({
          ...
          key: fromIdx !== -1 ? args[fromIdx + 1] : process.env.UIZZE_CONCEPT_SEED || crypto.randomBytes(4).toString("hex"),
