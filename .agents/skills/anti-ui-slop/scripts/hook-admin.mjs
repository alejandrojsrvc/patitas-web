...
const HOOK_MANIFEST_TARGETS = [
  {
    ...
    manifest: () => ({
      description: "Uizze design detector: immediate-tier checks after Edit/Write/MultiEdit on UI files, full-rule deep pass on Stop.",
      hooks: {
        PostToolUse: [
          {
            ...
            hooks: [
              {
                type: "command",
                command: 'node "${CLAUDE_PROJECT_DIR}/.claude/skills/uizze/scripts/hook.mjs"',
        ...
        Stop: [stopManifestEntry('node "${CLAUDE_PROJECT_DIR}/.claude/skills/uizze/scripts/hook.mjs"')],
      },
    }),
  },
  {
    ...
    manifest: () => ({
      hooks: {
        ...
        Stop: [stopManifestEntry('node ".agents/skills/uizze/scripts/hook.mjs"')],
      },
    }),
  },
  {
...
function statusReport(cwd) {
  ...
  const cfgPath = path.relative(cwd, getConfigPath(cwd)) || ".uizze/config.json";
  const localPath = path.relative(cwd, getLocalConfigPath(cwd)) || ".uizze/config.local.json";
  const cachePath = path.relative(cwd, getCachePath(cwd)) || ".uizze/hook.cache.json";
...
function valueHasUizzeHookMarker(value) {
  ...
  if (value && typeof value === "object") return Object.values(value).some(valueHasUizzeHookMarker);
  return false;
}
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error(`Pass a rule id, e.g. ${UIZZE_COMMAND} hooks ignore-rule side-tab`);
...
function addIgnoreFile(cwd, args) {
  ...
  if (!glob) throw new Error(`Pass a glob, e.g. ${UIZZE_COMMAND} hooks ignore-file "src/legacy/**"`);
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error(`Pass a rule id and value, e.g. ${UIZZE_COMMAND} hooks ignore-value overused-font Inter`);
  }
