...
(function () {
  ...
  const VOCAB = Array.isArray(window.__UIZZE_VOCAB__) ? window.__UIZZE_VOCAB__ : [];
  const ICONS = {};
  ...
  const LIVE_CHROME_MOUNT_CONTRACT = Array.isArray(window.__UIZZE_LIVE_MOUNT_CONTRACT__)
    ? window.__UIZZE_LIVE_MOUNT_CONTRACT__
    : ["root", "transport", "state", "actions"];
  const LIVE_UI_SURFACES = Array.isArray(window.__UIZZE_LIVE_UI_SURFACES__) ? window.__UIZZE_LIVE_UI_SURFACES__ : [];
  ...
  if (!domHelpers) {
    console.error("[uizze] live-browser-dom.js was not loaded. Live mode cannot start safely.");
    window.__UIZZE_LIVE_INIT__ = false;
    return;
  }
  const {
  ...
  function onAnnotDown(e) {
    ...
    const resizeEdge = e.target.closest?.("[data-uizze-placeholder-resize]")?.dataset.uizzePlaceholderResize;
  ...
  function stripManualEditRuntimeState(root) {
    ...
    const nodes = [root, ...root.querySelectorAll("[data-uizze-editable], [data-uizze-original-text], [data-uizze-text-wrap]")];
    for (const node of nodes) {
      const runtimeEditable = node.hasAttribute("data-uizze-editable") || node.hasAttribute("data-uizze-original-text");
  ...
  function resolveBarAnchor() {
    ...
    if (currentSessionId && (state === "GENERATING" || state === "CYCLING")) {
      ...
      if (wrapper) {
        const variantCount = wrapper.querySelectorAll('[data-uizze-variant]:not([data-uizze-variant="original"])').length;
  ...
  function recoverEmptyCycling(reason) {
    ...
    try {
      console.warn("[uizze] Refusing to render empty variant cycling state:", reason);
  ...
  function nearbyEditableTextsForManualEdit(rows, activeEl, originalText, newText) {
    ...
    for (const row of rows || []) {
      ...
      out.push({
        ...
        classes: row.el?.classList ? [...row.el.classList].filter((cls) => cls.indexOf("uizze-") !== 0) : [],
        text,
      });
  ...
  function restoreDiscardedManualEdits(entries) {
    ...
    if (failures > 0) {
      console.warn("[uizze] skipped unsafe copy edit DOM restore for", failures, "edit(s). Refresh to reset the page DOM.");
    }
    return failures;
  }
  ...
  function directMixedTextRestoreNodes(parent) {
    return Array.from(parent?.childNodes || []).filter((node) => {
      ...
      return node.nodeType === 1 && node.dataset && node.dataset.uizzeTextWrap === "true" && /\S/.test(node.textContent || "");
    });
  }
  ...
  function mountedParameterCount() {
    ...
    const wrapper = document.querySelector('[data-uizze-variants="' + currentSessionId + '"]');
    if (!wrapper) return 0;
    return [...wrapper.querySelectorAll('[data-uizze-variant]:not([data-uizze-variant="original"])')].reduce(
  ...
  function parseOriginalMarkupElement(originalMarkup) {
    ...
    const doc = parser.parseFromString('<div id="uizze-anchor">' + originalMarkup + "</div>", "text/html");
  ...
  function isUsableInjectionAnchor(el) {
    return !!el && el.parentElement && document.body.contains(el) && !own(el) && !el.closest?.("[data-uizze-variants]");
  }
  ...
  function waitForVariantAnchorAndRetry({ filePath, sessionId, srcWrapper, checkpointReason }) {
    ...
    const origContent = srcWrapper?.querySelector('[data-uizze-variant="original"] > :first-child');
    ...
    pendingVariantAnchorRetryObserver = new MutationObserver(() => {
      ...
      const wrapperLanded = !!document.querySelector('[data-uizze-variants="' + sessionId + '"]');
      if (!wrapperLanded) {
    ...
    for (const entry of contract) {
      if (entry.kind === "collection" && entry.item && entry.item.rootTag) {
        ...
        if (entry.probe && entry.probe.tag) {
          ...
          try {
            values[entry.prop] =
              liveEl.classList.contains(entry.probe.className) || !!liveEl.querySelector("." + cssEscapeIdent(entry.probe.className));
          } catch {
            /* keep default */
          }
        }
  ...
  async function mountSvelteComponentVariant(variantNum) {
    ...
    try {
      ...
      console.error("[uizze] Failed to mount component variant " + variantNum + " for " + sessionId + ":", err);
  ...
  async function injectSvelteComponentsFromManifest(manifestPath, sessionId) {
    ...
    try {
      ...
      wrapper.dataset.uizzeVariantCount = String(manifest.count || expectedVariants || 1);
      ...
      saveSession();
      if (parameterGenerationState === "loading") completeParameterPublication();
      console.log("[uizze] Mounted " + arrivedVariants + " " + manifest.framework + " component variants.");
    } catch (err) {
  ...
  function renderMountErrorCard() {
    ...
    title.textContent = mountErrorState.variant > 0 ? "Variant " + mountErrorState.variant + " failed to load" : "Variants failed to load";
  ...
  function abortSvelteComponentInjection(sessionId, details) {
    try {
      if (svelteComponentSession?.sessionId === sessionId) {
        ...
        const orphan = document.querySelector('[data-uizze-variants="' + sessionId + '"]');
  ...
  function discardOrphanedSession(reason) {
    ...
    if (!sessionId) return;
    console.warn("[uizze] Discarding orphaned session " + sessionId + ": " + reason);
  ...
  function injectVariantsFromSource(filePath, sessionId, opts = {}) {
    ...
    fetch(url)
      ...
      .then((html) => {
        ...
        const variants = wrapper.querySelectorAll('[data-uizze-variant]:not([data-uizze-variant="original"])');
        ...
        if (arrivedVariants <= 0) {
          if (state === "GENERATING") {
            ...
            if (!opts.generationCompleted) {
              console.log("[uizze] Source has scaffold but no variants yet; still generating.");
              return;
            }
  ...
  function variantStateSelector(sessionId, num) {
    ...
    const variant = num == null ? "[data-uizze-variant]" : '[data-uizze-variant="' + num + '"]';
  ...
  function updateVariantStateStylesheet(sessionId, num) {
    ...
    const hideOthers = variantStateSelector(sessionId) + ':not([data-uizze-variant="' + num + '"]) { ' + VARIANT_HIDE_DECL + " }";
    ...
    scrollLockObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        ...
        for (const n of m.addedNodes) {
          if (
            n.nodeType === 1 &&
            (n.matches?.('[data-uizze-variants="' + sessionId + '"]') || n.querySelector?.('[data-uizze-variants="' + sessionId + '"]'))
          ) {
  ...
  function connectSSE() {
      ...
      switch (msg.type) {
        case "connected":
          ...
          if (!hasProjectContext)
            showToast(`No PRODUCT.md found. Variants will be brand-agnostic. Run ${UIZZE_COMMAND} init to generate one.`, 7000);
    ...
    evtSource.onerror = () => {
      ...
      if (sseRetries <= SSE_MAX_RETRIES) {
        console.log("[uizze] SSE connection lost. Retry " + sseRetries + "/" + SSE_MAX_RETRIES + "...");
  ...
  function abandonForeignSession(sessionId) {
    ...
    console.warn("[uizze] The live server has no record of session " + sessionId + "; clearing stale local state.");
    markSessionHandled();
    cleanup({ instantChrome: true });
    ...
    try {
      ...
      if (shouldUseAncestorCropShaderProxy(el)) {
        try {
          ...
          console.warn("[uizze] Svelte ancestor crop capture failed, falling back to element capture:", err);
        }
      }
...
void main() {
  ...
  async function handleAccept() {
    ...
    const acceptWrapper = document.querySelector('[data-uizze-variants="' + currentSessionId + '"]');
    ...
    const acceptedIsSvelteComponent =
      svelteComponentSession?.sessionId === acceptedSessionId || isFrameworkComponentPreviewMode(acceptWrapper?.dataset?.uizzePreview);
    ...
    sendEvent(acceptPayload, { throwOnError: true })
      .then(() => {
        ...
        document.documentElement.dataset.uizzeAcceptToPickingMs = String(Date.now() - acceptPayload.clientSentAt);
  ...
  function snapshotAcceptedVariantDom(sessionId, variantId) {
    const wrapper = document.querySelector('[data-uizze-variants="' + sessionId + '"]');
    const accepted = wrapper?.querySelector?.('[data-uizze-variant="' + variantId + '"]');
  ...
  function acceptedDomAlreadyClean(pending) {
    ...
    return matches.length > 0 && matches.every((el) => !el.closest("[data-uizze-variants],[data-uizze-variant],[data-uizze-carbonize]"));
  }
  ...
  function findAcceptedRuntimeWrappers(sessionId) {
    ...
    return [
      ...new Set([
        ...document.querySelectorAll('[data-uizze-variants="' + sessionId + '"]'),
        ...document.querySelectorAll('[data-uizze-carbonize="' + sessionId + '"]'),
      ]),
    ];
  }
  ...
  function reloadAfterMissingAcceptedDom(pending) {
    if (acceptedDomAlreadyClean(pending)) return;
    if (pending?.id && document.querySelector('[data-uizze-variants="' + pending.id + '"]')) return;
    location.reload();
  }
  ...
  function commitAcceptedVariantToDom(sessionId, variantId) {
    ...
    if (style && !document.querySelector('style[data-uizze-accepted-css="' + sessionId + '"]')) {
  ...
  function restoreFromActiveSessions(activeSessions, reason) {
    ...
    if (wrapper && !isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview)) return false;
  ...
  function recoverMissedGenerationCompletion(activeSessions) {
    ...
    if (!file) return;
    console.log("[uizze] Reconnected after generation completed; recovering variants from source.");
    setTimeout(() => {
  ...
  function loadSession() {
    ...
    if (saved?.appRoot && APP_ROOT && saved.appRoot !== APP_ROOT) {
      console.warn("[uizze] Ignoring saved live session from another project (" + saved.appRoot + ").");
  ...
  function cleanup(options) {
    ...
    if (svelteComponentSession?.sessionId === cleanupSessionId) {
      ...
      setTimeout(function () {
        ...
        const lateWrapper = document.querySelector('[data-uizze-variants="' + cleanupSessionId + '"]');
        if (!lateWrapper) return;
        const orig = lateWrapper.querySelector('[data-uizze-variant="original"]');
        if (orig) {
  ...
  function resumeSession() {
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview) && svelteComponentSession?.sessionId !== sessionId) {
      wrapper.remove();
    ...
    if (isFrameworkComponentPreviewMode(wrapper.dataset.uizzePreview)) {
      ...
      currentSessionId = sessionId;
      expectedVariants =
        Number(wrapper.dataset.uizzeVariantCount) || Number(svelteComponentSession.manifest?.count) || expectedVariants || 1;
  ...
  function syncAgentPollingUi(connected) {
    ...
    globalBarBrandEl.setAttribute(
      "aria-label",
      agentStatusMessage ? "Uizze live mode - " + (agentHasWorkInFlight() ? "agent is working" : "agent not polling") : "Uizze live mode",
    );
  ...
  function designEmptyMessage() {
    if (designState.hasMd && !designState.hasSidecar) {
      return (
        "DESIGN.md found, no structured tokens to display. Run " + UIZZE_COMMAND + " document to generate the .uizze/design.json sidecar."
      );
    }
  ...
  function init() {
    ...
    if (!resumeSession()) {
      ...
      const scout = new MutationObserver(() => {
        ...
        scout.disconnect();
        if (resumeSession()) {
          console.log("[uizze] Resumed deferred session " + currentSessionId + " (post-hydration).");
        }
      });
      scout.observe(document.body, { childList: true, subtree: true });
    } else {
      console.log(
        "[uizze] Resumed active variant session " + currentSessionId + " (" + arrivedVariants + "/" + expectedVariants + " variants).",
      );
    }
