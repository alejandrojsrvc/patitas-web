...
The agent should insert variant HTML at insertLine.`);
  ...
  const useSvelteComponent = resolveSourceTraits(targetFile).preview === "component" && shouldUseSvelteComponentInjection(targetFile);
...
function detectCommentSyntax(filePath) {
  return resolveSourceTraits(filePath).commentSyntax === "jsx" ? { open: "{/*", close: "*/}" } : { open: "<!--", close: "-->" };
}
