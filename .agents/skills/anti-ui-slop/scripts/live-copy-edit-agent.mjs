...
export function runCopyEditPostApplyChecks({ cwd = process.cwd(), files = [] } = {}) {
  ...
  for (const relativeFile of uniqueFiles) {
    ...
    if (markerMatch) failures.push({ file: relativeFile, reason: "leftover_uizze_marker", marker: markerMatch });
...
function stripLiveRuntimeHtml(html) {
  ...
  return html
    .replace(/\sdata-uizze-(?:original-text|editable|text-wrap)(?:=(?:"[^"]*"|'[^']*'|[^\s>]+))?/g, "")
...
export function describeNoProviderError({ exists = commandExists, chatAvailable = () => false, env = process.env } = {}) {
  ...
  if (chatAvailable()) {
    lines.push("  • Chat: an Uizze live session is polling but selection chose another provider — unexpected; please report.");
  } else {
    lines.push(
